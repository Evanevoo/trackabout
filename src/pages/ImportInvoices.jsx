  ...
  return (
    <div className="max-w-3xl mx-auto mt-10 bg-white shadow-lg rounded-2xl p-8 border border-gray-200">
      <div className="flex justify-between items-center mb-8">
        <h2 className="text-2xl font-bold text-gray-900 tracking-tight">Import Invoices Only</h2>
        <button
          onClick={() => navigate(-1)}
      ...
      <form onSubmit={handleImport} className="mb-6 flex gap-2 items-end">
        ...
        <button type="submit" className="bg-blue-700 text-white px-6 py-2 rounded-full font-semibold shadow-sm hover:bg-blue-800 transition text-sm" disabled={!file || !preview.length || loading || !previewChecked || validationErrors.length > 0 || importing}>{loading ? 'Importing...' : 'Import'}</button>
      </form>
      {/* Field Mapping UI */}
      {columns.length > 0 && (
            ...
            <table className="min-w-full bg-white border border-gray-200 text-xs">
              <thead>
                <tr>
                  ...
                  <th className="border-b border-gray-100 px-2 py-1 text-xs font-semibold text-gray-700 text-left">Invoice</th>
                ...
                {preview.slice(0, 5).map((row, i) => {
                  ...
                  return (
                    <tr key={i} className={rowErrors.length ? 'bg-red-50' : ''}>
                      ...
                      <td className="px-2 py-1 text-xs text-gray-700 whitespace-nowrap">{status.invoiceStatus || ''}</td>
